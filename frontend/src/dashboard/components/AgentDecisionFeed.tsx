            ...
            return (
              <Box key={row.id} sx={{ py: 2 }}>
                <Stack direction="row" gap={1} alignItems="center" flexWrap="wrap" mb={1}>
                  ...
                  <Chip
                    size="small"
                    label={approvalLabel(row.approved)}
                    color={approvalColor(row.approved)}
                  />
                ...
                <Stack direction="row" gap={1} alignItems="center" flexWrap="wrap" mt={1.25}>
                  <Chip
                    size="small"
                    variant="outlined"
                    label={`hash ${truncateHash(row.decision_hash)}`}
                  />
                  {confidence !== null ? (
                    <Chip
                      size="small"
                  ...
                  {row.reviewed_by ? (
                    <Chip
                      size="small"
                      variant="outlined"
                      label={`reviewed by ${row.reviewed_by}`}
                    />
                  ) : null}
                </Stack>
                ...
                {row.attestation?.accountHash ? (
                  ...
                  >
                    Attested by {row.attestation.agent} ·{' '}
                    {truncateHash(row.attestation.accountHash)}
                  </MuiLink>
                ) : null}
              </Box>
            )
