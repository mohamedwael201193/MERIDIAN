...
export function useAuditSummaries(limit = 20) {
  return useSWR(['audit-summaries', limit], () =>
...
export async function revalidateMeridianData() {
  await mutate(
    key =>
      typeof key === 'string' ||
      (Array.isArray(key) &&
        ['tokens', 'token-yield', 'yield-history', 'holders', 'events', 'audit-summaries', 'decisions', 'health', 'ready', 'holder-compliance'].includes(
          String(key[0]),
        )),
  );
}
