export const MERIDIAN_NETWORK = process.env.NEXT_PUBLIC_CASPER_NETWORK ?? 'casper-test'
...
export function explorerAccountUrl(accountHash: string): string {
  const hash = accountHash.startsWith('account-hash-') ? accountHash : `account-hash-${accountHash}`
