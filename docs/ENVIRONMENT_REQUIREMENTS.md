...
- **Purpose:** Deployer's private key PEM material for deployment scripts and x402 settlement.
- **Why we need it:** Signs contract deployments (local/CI) and x402 facilitator `/settle` native transfers.
- **Where to obtain:** Generated by `casper-client keygen` (see 2.1).
- **Format:** **Inline PEM string** in `.env` / Render env (single line with `\n` escapes). **File paths are rejected at runtime.**
- **Example (local `.env`):**
  ```bash
  MERIDIAN_DEPLOYER_PRIVATE_KEY_PEM=-----BEGIN EC PRIVATE KEY-----\nMIGk...\n-----END EC PRIVATE KEY-----\n
  ```
- **Migration from file paths:** `node scripts/inline-pem-env.mjs`
- **⚠️ SECURITY:** NEVER commit PEM content. NEVER log PEM values. Used only by x402 facilitator on Render — **not** by AI agents.
- **Verification:**
  ```bash
  node scripts/verify-agent-identity.mjs   # includes deployer wallet check
  bash scripts/verify-env.sh               # confirms inline BEGIN marker
  ```
...
Each of the 3 AI agents has its **own independent Casper wallet** with separate public key, account hash, and inline PEM. Full spec: `docs/AGENT_IDENTITY.md`.

| Agent | Public key | Account hash | Private key (inline PEM) |
|-------|------------|--------------|--------------------------|
...
Agents sign **decision attestations** with their own keys. The deployer key is never loaded in agent processes.

### 3.1–3.6 Agent variables

All `*_PRIVATE_KEY_PEM` values must be **inline PEM** (single line, `\n` escapes). File paths are rejected.

Migration: `node scripts/inline-pem-env.mjs`
...
Expected: 10/10 identity checks pass; inline PEM markers present for all 4 wallets (deployer + 3 agents).
